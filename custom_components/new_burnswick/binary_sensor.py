...
# New Brunswick is always Atlantic Time, regardless of the HA instance's
# timezone setting
...
def _fire_allowed_now(category: int) -> bool | None:
    ...
    if category == _CATEGORY_YELLOW:
        # Allowed from 8 PM (inclusive) through midnight and up to
        # (not including) 8 AM
        return now_time >= datetime.time(
            _ALLOW_HOUR_START, 0
        ) or now_time < datetime.time(_ALLOW_HOUR_END, 0)
...
def _next_transition_time() -> datetime.datetime:
    """Return next 8 AM, 2 PM, or 8 PM as a timezone-aware Atlantic Time."""
    now_nb = datetime.datetime.now(tz=NB_TZ)
    today = now_nb.date()
    candidates = [
        ...
        datetime.datetime.combine(
            ...
            datetime.time(_ALLOW_HOUR_START, 0),
            NB_TZ,
        ),
    ]
...
class NewBurnswickFireAllowedSensor(
    ...
    def _handle_time_transition(self, now: datetime.datetime) -> None:
        ...
        _LOGGER.debug(
            "%s: Policy transition boundary reached. Updating state.", self.entity_id
        )
    ...
    def extra_state_attributes(self) -> dict[str, Any]:
        ...
        return {
            ...
            "time_window": (
                f"{_ALLOW_HOUR_START}:00 – {_ALLOW_HOUR_END:02d}:00 "
                "(yellow status only)"
            ),
